...
import { Play, SkipForward, RotateCcw, CheckCircle2, Undo2, Redo2, Loader2, GitBranch, ArrowRight, Zap } from 'lucide-react';
        ...
        <>
          ...
          <div className="flex gap-1">
            <button
              onClick={onUndo}
              disabled={!canUndo}
              className={`flex items-center gap-2 px-6 py-3 rounded-l-full font-bold transition-all border-y border-l ${
                !canUndo 
                  ? 'bg-slate-50 dark:bg-slate-900/20 border-slate-200 dark:border-slate-800 text-slate-300 dark:text-slate-700 cursor-not-allowed' 
                  : isDrawing
                    ...
                    : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-amber-50 dark:hover:bg-amber-900/20 text-amber-600 dark:text-amber-400'
              }`}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-r-full font-bold transition-all border ${
                !canRedo || isDrawing
                  ? 'bg-slate-50 dark:bg-slate-900/20 border-slate-200 dark:border-slate-800 text-slate-300 dark:text-slate-700 cursor-not-allowed' 
                  : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400'
              }`}
              title="Redo Move"
            >
            ...
            <>
              <div className="flex gap-1">
                <button
                  onClick={onUndo}
                  disabled={!canUndo}
                  className="flex items-center gap-2 px-6 py-3 rounded-l-full font-bold transition-all border-y border-l bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:bg-amber-50 dark:hover:bg-amber-900/20 text-amber-600 dark:text-amber-400"
                >
                  <Undo2 size={20} />
                  Undo
                </button>
                <button
                  onClick={onRedo}
                  disabled={!canRedo}
                  className={`flex items-center gap-2 px-6 py-3 rounded-r-full font-bold transition-all border bg-white dark:bg-slate-800 ${!canRedo ? 'text-slate-300 border-slate-200 dark:border-slate-800 opacity-50' : 'border-slate-200 dark:border-slate-700 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400'}`}
                >
                  Redo
                  <Redo2 size={20} />
                </button>
