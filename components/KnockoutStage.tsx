...
export const KnockoutStage: React.FC<KnockoutStageProps> = ({ groups, simulation }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      {/* FIXED HEIGHT DASHBOARD - Prevents layout shifts and scrolling jumping */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-6 lg:p-8 shadow-xl dark:shadow-none relative overflow-hidden h-[300px] flex flex-col justify-between">
        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
          ...
          {/* STATISTICS GRID - Enforced strict height to block any scroll jitter */}
      ...
      <div className="mt-8">
        ...
        {activeTab === 'tree' && (
          <div className="relative overflow-x-auto pb-12 custom-scrollbar animate-in fade-in duration-500 rounded-3xl bg-slate-200/10 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/50">
...
const TeamStatsTooltip: React.FC<{ team: GroupStanding, x: number, y: number }> = ({ team, x, y }) => {
  ...
  return (
    <div 
      className="fixed z-[100] w-[280px] bg-slate-900/98 border border-white/10 rounded-3xl shadow-2xl p-5 pointer-events-none backdrop-blur-md"
      style={{ 
        left: Math.max(16, Math.min(window.innerWidth - 300, x + 20)), 
        top: Math.max(16, Math.min(window.innerHeight - 300, y - 60)) 
      }}
    >
...
const R32MatchCard: React.FC<{ 
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 border transition-colors rounded-2xl shadow-sm overflow-hidden flex flex-col ${hasWinner ? 'border-indigo-500/40' : 'border-slate-200 dark:border-slate-800'}`}>
